...
function delPro($id){
	$where="id=$id";  //id=1  條件
	$res=delete("imooc_pro",$where);  //刪除id這條
	$proImgs=getAllImgByProId($id);   //獲取id對應的圖片路徑
	//print_r($proImgs);
	if($proImgs&&is_array($proImgs)){   //判斷有值并且是数组
		foreach($proImgs as $proImg){    //循环数组
			if(file_exists("uploads/".$proImg['albumPath'])){    //file_exists — 检查文件或目录是否存在  如果存在
				unlink("uploads/".$proImg['albumPath']);    //unlink — 删除文件
			}
